...
export class EnvironmentNode extends vscode.TreeItem {
  ...
  async getSecrets(): Promise<
    (EnvironmentSecretNode | EmptyEnvironmentSecretsNode)[]
  > {
    const result =
      await this.gitHubRepoContext.client.actions.listEnvironmentSecrets({
    ...
    return data.map(
      (s) => new EnvironmentSecretNode(this.gitHubRepoContext, s)
    );
  }
}
